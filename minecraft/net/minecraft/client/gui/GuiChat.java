...
public class GuiChat extends GuiScreen
{
	...
	protected void keyTyped(char par1, int par2)
	{
		...
		else
		{
			...
			// Check if this is a Wolfpack chat command
			if (!var3.substring(0, 1).equalsIgnoreCase(".")) {
				...
				// It is a chat command
				String[] commandWithArgs = var3.substring(1).split(" ");
				for (Module mod: Wolfpack.getModuleManager().moduleList) {
					if (mod.isChatCommand()) {
						for (String command: mod.getChatCommands()) {
							if (command.toLowerCase().trim().equals(commandWithArgs[0].toLowerCase().trim())) {
								mod.onChatCommand(commandWithArgs);
							}
						}
					}
				}
